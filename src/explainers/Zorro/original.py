...
from numpy import nan, array, savez_compressed, load, inf
from torch import is_tensor, no_grad, ones
from torch import zeros_like, zeros, as_tensor, cat, Generator, gather, int64, randint
...
class Zorro(Module):
    ...
    def __num_hops__(self):
        return self.model.get_num_hops()
    ...
    def argmax_distortion_general(self,
        ...
        if self.greedy:
            ...
            if initialization:
                ...
                restricted_possible_elements = zeros_like(possible_elements, device=self.device)
    ...
    def _determine_minimal_set(self, initial_distortion, tau, possible_nodes, possible_features,
        ...
        if self.record_process_time:
            last_time = time.time()
            executed_selections = [[nan, nan, current_distortion, 0]]
        else:
            last_time = 0
            executed_selections = [[nan, nan, current_distortion]]
    ...
    def recursively_get_minimal_sets(self, initial_distortion, tau, possible_nodes, possible_features,
                                     recursion_depth=inf, save_initial_improve=False):
        ...
        reduced_nodes = possible_nodes - as_tensor(minimal_nodes_and_features_sets[0][0], device=self.device)
        reduced_features = possible_features - as_tensor(minimal_nodes_and_features_sets[0][1],
                                                         device=self.device)
...
def save_minimal_nodes_and_features_sets(save_path, node, minimal_nodes_and_features_sets,
    ...
    if minimal_nodes_and_features_sets is None:
        ...
        numpy_dict = {
            "node": array(node),
            "number_of_sets": array(len(minimal_nodes_and_features_sets)),
        }
        ...
        for i, (selected_nodes, selected_features, executed_selections) in enumerate(minimal_nodes_and_features_sets):
            ...
            numpy_dict[selection_label + str(i)] = array(executed_selections)

    if initial_node_improve is not None:
        numpy_dict["initial_node_improve"] = array(initial_node_improve)

    if initial_feature_improve is not None:
        numpy_dict["initial_feature_improve"] = array(initial_feature_improve)

    savez_compressed(path, **numpy_dict)
...
def load_minimal_nodes_and_features_sets(path_prefix, node, check_for_initial_improves=False):
    ...
    save = load(path, allow_pickle=False)
...
def distortion(model, node_idx=None, full_feature_matrix=None, computation_graph_feature_matrix=None,
    ...
    if validity:
        samples = 1
        full_feature_matrix = zeros_like(full_feature_matrix)

    correct = 0.0

    rng = Generator(device=device)
    rng.manual_seed(random_seed)
    random_indices = randint(num_nodes, (samples, num_nodes_computation_graph, num_features),
                             generator=rng,
                             device=device,
                             )
    random_indices = random_indices.type(int64)

    for i in range(samples):
        random_features = gather(full_feature_matrix,
                                 dim=0,
